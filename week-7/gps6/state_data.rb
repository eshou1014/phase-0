...
#This hash has a nested hash inside of it. It contains a hash which holds all the states, another hash which holds each states "key" value, with another hash that contains its own key and value hash. By creating a key with the symbol method, it allows the key to be used as a symbol. The states themselves have keys using the =>. STATE_DATA is a Ruby Constant. It's scope is that it remains the same for the duration of the program and never changes.
